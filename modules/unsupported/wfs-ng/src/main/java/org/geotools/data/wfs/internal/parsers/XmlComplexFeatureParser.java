...
public class XmlComplexFeatureParser extends XmlFeatureParser<FeatureType, Feature> {
    ...
    public XmlComplexFeatureParser(InputStream getFeatureResponseStream, FeatureType targetType,
    ...
    public ReturnAttribute parseNextAttribute(ComplexType complexType)
        ...
        if (tagType == XmlPullParser.START_TAG) {
            // 2a. A start tag has been found; if it belongs to the complexType then we should parse it and return it.

            // 3. Convert the tag's name in to a NameImpl and then see if there's a descriptor by that name in the type:
            ...
            if (descriptor != null) {
                // 3a. We've found a descriptor for the tag's name in the complexType.

                // Get the type that the descriptor relates to, and get the GML Id if it's set:
                PropertyType type = descriptor.getType();
                String id = parser.getAttributeValue(GML.id.getNamespaceURI(),
                        GML.id.getLocalPart());

                // 4. Parse the tag's contents based on whether it's a ComplexType or an AttributeType.
                if (type instanceof ComplexType) {
                    // 4a. The element is a complex type so we must loop through each of its internal
                    ...
                    else
                    {
                        ...
                        while ((innerAttribute = parseNextAttribute((ComplexType) type)) != null) {
                            ...
                            if (ComplexAttribute.class.isAssignableFrom(innerAttribute.value.getClass())) {
                                ...
                                attributeBuilder.add(innerAttribute.id, innerAttribute.value,
                                        innerAttribute.name);
                            }
                        }
                    ...
                    // 4b. It's just a simple type so we can use super's parseAttributeValue method.
