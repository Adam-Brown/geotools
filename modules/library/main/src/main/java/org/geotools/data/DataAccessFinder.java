...
public final class DataAccessFinder {
	...
	static DataAccess<? extends FeatureType, ? extends Feature> getDataStore(
			...
			Iterator<? extends DataAccessFactory> ps) throws IOException {
		...
		while (ps.hasNext()) {
			...
			try {
				...
				LOGGER.log(Level.WARNING,
						"Problem asking " + fac.getDisplayName()
			...
			if (canProcess) {
				...
				try {
					...
					LOGGER.log(Level.WARNING,
							"Difficulity checking if " + fac.getDisplayName()
									+ " is available:" + t, t);
					// Protect against DataStores that don't carefully code
					// isAvailable
					continue;
				...
				if (isAvailable) {
					try {
						...
						LOGGER.log(Level.WARNING, fac.getDisplayName()
								+ " should be used, but could not connect",
								couldNotConnect);
					}
				} else {
					...
					LOGGER.log(Level.WARNING, fac.getDisplayName()
							+ " should be used, but is not availble",
	...
	public static synchronized Iterator<DataAccessFactory> getAllDataStores() {
		...
		Iterator<DataStoreFactorySpi> allDataStores = DataStoreFinder
				.getAllDataStores();
		Iterator<DataAccessFactory> allDataAccess = getAllDataStores(
				getServiceRegistry(), DataAccessFactory.class);
	...
	public static synchronized Iterator<DataAccessFactory> getAvailableDataStores() {
		...
		Set<DataAccessFactory> availableDS = getAvailableDataStores(
				serviceRegistry, DataAccessFactory.class);
	...
	private static FactoryRegistry getServiceRegistry() {
		...
		if (registry == null) {
			registry = new FactoryCreator(
					Arrays.asList(new Class<?>[] { DataAccessFactory.class }));
		}
		return registry;
	}
	...
	/**
	 * Resets the factory finder and prepares for a new full scan of the SPI
	 * subsystems
	 */
