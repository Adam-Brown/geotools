...
/**
 * You can make feature wrappers for specific types by extending this class and
 * annotating the descendant class's fields with {@link XSDMapping} to show what
 * they correspond to in the XSD.
 */
public abstract class FeatureWrapper {
	...
	/**
	 * Gets the underlying complex attribute. That is, the complex attribute
	 * that was wrapped. NB: This could be a Feature.
	 * 
	...
	/**
	 * Sets the underlying complex attribute. That is, the complex attribute
	 * that was wrapped. NB: This could be a Feature.
	 * 
	 * @param underlyingComplexAttribute
	 */
	private void setUnderlyingComplexAttribute(
			ComplexAttribute underlyingComplexAttribute) {
	...
	public static <T extends FeatureWrapper> T Wrap(
		...
		try {
			...
			XSDMapping classLevelXSDMapping = clazz
					.getAnnotation(XSDMapping.class);
			...
			for (Field field : clazz.getFields()) {
				...
				if (xsdMapping != null) {
					...
					Name xsdName = new NameImpl(namespace, separator,
							xsdMapping.local());
					...
					if (FeatureWrapper.class.isAssignableFrom(fieldType)) {
						...
						if (FeatureWrapper.class
								.isAssignableFrom(collectionType)) {
							// The collection is complex.
							for (Property property : targetAttribute
									.getProperties(xsdName)) {
								collection
										.add(Wrap(
						...
						Property property = targetAttribute
								.getProperty(xsdName);
