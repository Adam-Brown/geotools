 ...
 * 
 * <li>In an <a
 * href="http://www.oasis-open.org/committees/entity/spec-2001-08-06.html">OASIS
 * Catalog</a> (with URI resolution semantics), which maps URLs to arbitrary
 * filesystem locations.</li>
 * 
 * <li>On the classpath, where resources are located by their Simple HTTP
 * Resource Path (see {@link #getSimpleHttpResourcePath(URI)}).
 * 
...
public class AppSchemaResolver {
	...
	/**
	 * Maps a resolved location (a URL used to obtain a schema from a file or
	 * the classpath) to the original HTTP URL used to obtain it. This is
	 * required so that relative imports can be resolved if they cross
	 * resolution boundaries. For example, an import ../../../om/.. used to
	 * import om in a schema, where one is supplied locally and the other must
	 * be downloaded and cached. Another example is when the schemas are in
	 * different jar files.
	 */
	...
	/**
	 * Convenience constructor for a resolver with neither catalog nor cache
	 * (just classpath).
	 */
	...
	/**
	 * Resolve an absolute or relative URL to a local file or jar URL. Relative
	 * URLs are resolved against a context schema URL if provided.
	 * 
	 * @param location
	 *            an absolute or relative URL for a schema
	 * @param context
	 *            an absolute URL specifying the context schema of a relative
	...
	public String resolve(String location, String context) {
		...
		if (!locationUri.isAbsolute()) {
			...
			String originalContext = resolvedLocationToOriginalLocationMap
					.get(context);
	...
	public String resolve(String location) {
		...
		LOGGER.fine(String.format("Resolved %s -> %s", location,
				resolvedLocation));
		return resolvedLocation;
	}

	/**
	 * Return the Simple HTTP Resource Path for an absolute http/https URL. Does
	 * not include query components in the path.
	 * 
	 * @param location
	 *            not null
	 ...
	 *            not null
	 * @param keepQuery
	 *            indicates whether or not the query components should be
	 *            included in the path. If this is set to true then the query
	 *            portion is converted to an MD5 message digest and that string
	 *            is used to identify the file in the cache.
	 * @return the resource path with a leading slash
	 ...
	 * <p>
	 * 
	 * The Simple HTTP Resource Path maps an HTTP or HTTPS URL to a path on the
	 * classpath or relative to some other root. To form the Simple HTTP
	 * Resource Path from an http/https URL:
	 * 
	 * <ol>
	 * <li>Protocol, port, fragment, and query are ignored.</li>
	 * <li>Take the host name, split it into its components, reverse their
	 * order, prepend a forward slash to each, and concatenate them.</li>
	 * <li>Append the path component of the URL.</li>
	 * </ol>
	 * 
	 * For example <code>http://schemas.example.org/exampleml/exml.xsd</code>
	 * becomes <code>/org/example/schemas/exampleml/exml.xsd</code> .
	 * 
	 * <p>
	 * 
	 * The Simple HTTP Resource Path always starts with a forward slash (if not
	 * null). Does not include query components in the path.
	 * 
	 * @param location
	 *            not null
	 * @return the Simple HTTP Resource Path as a string, or null if the URI is
	...
	/**
	 * Return the URL for a resource found on the classpath at the Simple HTTP
	 * Resource Path. This allows (for example) schema documents in jar files to
	 * be loaded from the classpath using their canonical HTTP URLs.
	 * 
	 * @param location
	...
	/**
	 * Return the string representation of URL for a resource found on the
	 * classpath at the Simple HTTP Resource Path. This allows (for example)
	 * schema documents in jar files to be loaded from the classpath using their
	 * canonical HTTP URLs.
	 * 
	 * @param location
	 * @return the string representation of a classpath URL, or null if not
	 *         found
	 */
	 ...
	 * 
	 * @return An MD5 digest generated from message, this string is always 32
	 *         characters long. Or returns null if there was an error.
	 */
	private static String stringToMD5String(String message) {
		...
		return String.format("%032x",
				new BigInteger(1, md.digest(bytesOfMessage)));
	}
